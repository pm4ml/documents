...
NOTE: The simplified API between the {mojaloop-connector} and the {core-connector} is a synchronous HTTP API, while the interface between the {mojaloop-connector} and the Mojaloop Switch is the https://docs.mojaloop.io/mojaloop-specification/#api-definition[asynchronous Mojaloop FSPIOP API]. +
 +
